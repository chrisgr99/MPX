...
CHART opens the window, and closes it again. It opens clear of the module that opened it: the window is over the rack, and at a comfortable zoom a ten HP panel is wide enough that a window in the default place lands on top of it — leaving the button you just pressed underneath the thing it opened. Once you have dragged it somewhere yourself, that is where it opens, because a position you chose is worth more than one that was worked out.

**The window goes when the module does**, which includes every patch load rather than only a deletion. Left standing it would hold a pointer to a module that no longer exists and draw a chart out of freed memory, and the CHART button of some other chart module would be toggling a window belonging to a rack that is gone. The window is a child of the scene and therefore always over the rack, so bringing an open one to the front achieves nothing you can see — while a button labelled CHART that shuts the chart is what anybody would try. A second chart module asking for the window is the exception: that is not "close it", it is "show me mine instead". TEMPO is the large knob with the tempo in force written above it in green — the clock's measured rate when one is patched, the knob's otherwise, and correct while stopped.
